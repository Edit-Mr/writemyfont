...
async function loadSettings() {
  ...
  if (settings.customGlyphs && settings.customGlyphs != "") {
    // 如果有自定義文字，則添加到列表中
...
async function initCanvas(canvas) {
  ...
  if (settings.gridType == "stargrid") {
    ...
    gridCtx.beginPath();
    gridCtx.rect(
      gridXOff + emWidth * 0.15,
      gridYOff + emHeight * 0.15,
      emWidth * 0.7,
      emHeight * 0.7,
    );
    gridCtx.stroke();
  ...
  $("#pressureButton")
    .removeClass("on off")
    .addClass(settings.pressureMode ? "on" : "off");
}

function initListSelect($listSelect) {
  $listSelect.empty(); // 清空選單
  for (var list in glyphList) {
    $listSelect.append($("<option></option>").val(list).text(list));
  }
}
...
async function createFont(glyphs, gidMap, verts, ccmps) {
  ...
  for (let i in ccmps) {
    ...
    if (!allpass) continue;
    font.substitution.addLigature("ccmp", {
      sub: subfrom,
      by: gidMap[gname_to],
    });
  }
  ...
  for (let i in verts) {
    ...
    font.substitution.addSingle("vert", {
      sub: gidMap[gname_h],
      by: gidMap[gname_v],
    });
  }
...
$(document).ready(async function () {
  ...
  function setGlyph(index) {
    ...
    if (glyphMap[nowGlyph].v && nowGlyph.indexOf(".vert") > 0)
      $("#charSeq").addClass("vert");
  ...
  async function saveToLocalDB(runNow = false) {
    ...
    if (runNow) {
      ...
      // 延遲儲存
      svgTimers[saveGlyph] = setTimeout(async function () {
        // 延遲轉外框
    ...
    if (!breakFlag) {
      if (char.length == (char.codePointAt(0) < 65536 ? 1 : 2)) {
        if (confirm(fdrawer.notFound + "\n" + fdrawer.confirmAdd)) {
          ...
          updateSetting(
            "customGlyphs",
            glyphList[fdrawer.customList].join(","),
          ); // 儲存自定義字符
  ...
  function getPressureValue(mode, event, x, y) {
    ...
    if (mode == "move" && hasPointerEvent && !eventType.includes("pointer"))
      return null; // 如果曾經有pointer事件，則只接受pointer事件
    ...
    let touchForce =
      event.originalEvent.touches && event.originalEvent.touches.length > 0
        ? event.originalEvent.touches[0].force
        : null;
    let webkitForce =
      event.originalEvent.webkitForce !== undefined
        ? event.originalEvent.webkitForce
        : null;
    ...
    if (isRealPressure) {
      ...
      if (pressure > 0 && pressure < 1 && pressure != 0.5)
        hasRealPressure = true; // 出現過看似真實的筆壓值

      // 真實筆壓值套用敏感度運算
      if (settings.pressureEffect == "contrast")
        pressure = 0.5 + Math.sin((pressure * 0.9 - 0.45) * Math.PI) / 2;
      if (settings.pressureEffect == "enhance")
        pressure = Math.sin((pressure * Math.PI) / 2);
      if (settings.pressureEffect == "enhancex")
        pressure = Math.sin((Math.sin((pressure * Math.PI) / 2) * Math.PI) / 2);
      ...
      //if (simulatePressure && lastX && lastY) {
  ...
  $canvas.on("mousedown touchstart pointerdown", function (event) {
    ...
    if (!isDrawing && png != undoStack[undoStack.length - 1])
      undoStack.push(png); // 儲存當前畫布狀態到 undoStack
    isDrawing = true; // 儲存畫布後正式宣告筆畫開始
    if (svgTimers[nowGlyph]) clearTimeout(svgTimers[nowGlyph]); // 停止SVG轉外框 (提高效能)

    if (settings.oldPressureMode) {
      // 舊筆壓模式
      const pressure = pressureDrawing.simulatePressure(
        event.originalEvent,
        "start",
      );
      ...
      ctx.globalCompositeOperation = eraseMode
        ? "destination-out"
        : "source-over"; // 如果是橡皮擦模式，則使用 destination-out，否則使用 source-over
  ...
  $canvas.on("mousemove touchmove pointermove", function (event) {
    ...
    if (settings.oldPressureMode) {
      ...
      if (d > 40)
        events.push(
          `Long-DrawImage / ${pressureVal} / ${event.originalEvent.pointerType} / ${x}, ${y}, ${lw} (${lastX}, ${lastY}, ${lastLW}) ${d}`,
        ); // 儲存事件資訊
      if (d > 0) {
        for (var t = d; t > 0; t--) {
        ...
        events.push(
          `Move-DrawImage / ${pressureVal} / ${event.originalEvent.pointerType} / ${x}, ${y}, ${lw} (${lastX}, ${lastY}, ${lastLW}) ${d}`,
        ); // 儲存事件資訊
      }
  ...
  $(document)
    ...
    .on("touchend", function (event) {
      if (undoTouchTime && new Date().getTime() - undoTouchTime < 250) {
        // 如果雙指觸控時間夠短
  ...
  $("#downloadFontButton").on("click", async function () {
    ...
    const glyphs = [
      // 建立字符陣列，並加入一些空格字符（因程式機制上無法畫出空白字符，只能自動產生）
    ...
    for (let gname in glyphMap) {
      ...
      try {
        ...
        if (!svgData) continue;
        let path = await opentype.Path.fromSVG(svgData, {
          flipYBase: 0,
          scale: scale,
          y: 880 - scaleoff,
          x: scaleoff,
        ...
        let adw = upm;
        if (glyphMap[gname].w == "P" || glyphMap[gname].w == "H") {
          // 比例寬自動調整
          adw = padPath(path, 50);
        ...
        if (gname.match(/^uni([0-9A-F]{4})$/i)) {
          ...
          gname.indexOf(".vert") < 0 &&
          glyphMap[gname].c.length == 1
        ) {
        ...
        if (glyphMap[gname].f) {
          ...
          let pathF = await opentype.Path.fromSVG(svgData, {
            flipYBase: 0,
            scale: scale,
            y: 880 - scaleoff,
            x: scaleoff,
          ...
          let unicodeF = null;
          if (gnameF.match(/^uni([0-9A-F]{4})$/i))
            unicodeF = parseInt(RegExp.$1, 16); // 轉換為 Unicode 編碼
    ...
    link.href = window.URL.createObjectURL(new Blob([font.toArrayBuffer()]), {
      type: "font/opentype",
    });
    link.click();
  ...
  $("#settingButton").on("click", async function () {
    $("#settings-title").text(
      settings.notNewFlag ? fdrawer.settingsTitle : fdrawer.welcomeTitle,
    );
  ...
  $("#fontNameEng").on("change", function () {
    updateSetting(
      "fontNameEng",
      $(this)
        .val()
  ...
  $("#fontNameCJK").on("change", function () {
    updateSetting("fontNameCJK", $(this).val());
  });
  $("#smallModeCheck").on("click", async function () {
    await updateSetting("smallMode", $(this).prop("checked"));
    $("#canvas-container").toggleClass("smallmode", settings.smallMode);
  });
  $("#noFixedWidthFlag").on("click", function () {
    updateSetting("noFixedWidthFlag", $(this).prop("checked"));
  });
  ...
  $("#pressureEffectSelect").change(function () {
    updateSetting("pressureEffect", $(this).val());
  });
      ...
      if (svgData) {
        ...
        $("#listup-body").append(
          $('<svg version="1.1" viewBox="' + viewBox + '">')
            .html('<path d="' + svgData + '" stroke="#000" fill="#000"></path>')
            .data("index", i)
            .on("click", function () {
        ...
        var cell = $("<span>")
          .text(glyphMap[gname].c)
          .data("index", i)
          .on("click", function () {
            setGlyph($(this).data("index") * 1);
            $("#listup-container").hide();
          });
        if (glyphMap[gname].v && gname.indexOf(".vert") > 0)
          cell.addClass("vert");
  ...
  function getCanvasCoordinates(event) {
    ...
    const touch = event.type.includes("touch")
  ...
  $("#exportEventsButton").on("click", async function () {
    ...
    if (data.length > 0) {
      ...
      const link = document.createElement("a");
      link.download =
        settings.fontNameEng + "-EventLog" + new Date().toISOString() + ".txt";
  ...
  $("#exportDataButton").on("click", async function () {
    ...
    request.onsuccess = async function (event) {
      ...
      if (data.length > 0) {
        ...
        const link = document.createElement("a");
        link.download =
          settings.fontNameEng + "-" + new Date().toISOString() + ".txt";
        link.href = window.URL.createObjectURL(blob);
        link.click();
      } else {
        alert(fdrawer.noDataToExport);
